       ...
       $id=''; $factura='';  $codigo='';$nombre='';$cantidad='';$valor='';$importe='';    
        if(!empty($_GET['id'])){    
            ...
            if($dato=mysql_fetch_array($can)){
                $factura=$dato['factura'];$codigo=$dato['codigo'];$nombre=$dato['nombre'];$cantidad=$dato['cantidad'];$valor=$dato['valor'];$importe=$dato['importe'];
    ...
    <div class="container-fluid">
        <?php 
            if(!empty($_POST['id']) ) {
                ...
                $factura=$_POST['factura'];$codigo=$_POST['codigo'];$nombre=$_POST['nombre'];$cantidad=$_POST['cantidad'];$valor=$_POST['valor'];$importe=$_POST['importe'];


                $can=mysql_query("SELECT * FROM detalle WHERE  id=$id");
                if($dato=mysql_fetch_array($can)){
                    if($boton=='Actualizar Factura'){
                    $xSQL="Update detalle Set  codigo='$codigo',nombre='$nombre',cantidad='$cantidad',valor='$valor',importe='$importe'

                                                     Where id=$id";
                        mysql_query($xSQL); echo '  <div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">X</button>
                                  <strong>factura !</strong> Actualizado con Exito</div>';}
                    //               else{echo ' <div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">X</button><strong>Error! </strong>El numero de documento que ingreso le pertenece al cliente '.$dato['factura'].'</div>';
                    // }       
                }
                    ...
                    <div class="card-body">
                        ...
                        <form name="form1" method="post" action="">
                            <div class="row">
                                <div class="col-6">
                                    <div class="form-group">
                                        <label for="textfield">factura: </label>
                                            <input type="text" class="form-control form-control-sm" placeholder="Ingrese el nombre y apellido" name="factura" id="factura" <?php 

                                            if(!empty($factura)){

                                        } ?> value="<?php echo $factura; ?>" autocomplete="off" required>
                                    </div>
                                    <div class="form-group">
                                       
                                      <label for="textfield">codigo: </label><input type="text" name="codigo" class="form-control form-control-sm" placeholder="Ingrese cedula " id="codigo" value="<?php echo $codigo; ?>" autocomplete="off" required>  
                                    </div>
                                    <div class="form-group">
                                      <label for="textfield">Detalle: </label><input type="text" class="form-control form-control-sm" placeholder="Ingrese nombre" name="nombre" id="nombre" value="<?php echo $nombre; ?>" autocomplete="off">  
                                    </div>
                                ...
                                <div class="col-6">
                                    <div class="form-group">
                                        <label for="textfield">Cantidad: </label><input type="text" name="cantidad" id="cantidad" class="form-control form-control-sm" placeholder="Ingrese la cantidad" value="<?php echo $cantidad; ?>" autocomplete="off" >
                                    </div>
                                    <div class="form-group">
                                        <label class="textfield">Valor</label>
                                       <input type="text" name="valor" id="valor" class="form-control form-control-sm" placeholder="Ingrese el valor" value="<?php echo $valor; ?>" autocomplete="off"></input>
                                    </div>
                                     <div class="form-group">
                                         <label class="textfield">Importe</label>
                                       <input type="text" name="importe" id="importe" class="form-control form-control-sm" placeholder="Ingrese el importe" value="<?php echo $importe; ?>" autocomplete="off"></input>
                                    </div>
                            ...
                            <?php if($boton=='Actualizar factura'){ ?> <a href="crearfactura.php" class="btn btn-lg btn-danger">Cancelar</a><?php }  ?>
                            </div> 
                        </form>
                        <!-- formulario============== -->
                    </div>
