...
switch ($actionRequest) {
    ...
    case 'allInvoicesCompra':
        // $sql = "SELECT d.id, d.factura as factura, f.cajera as cajero, f.prov as cliente, f.estado, f.fecha, d.tipo, d.total
        // FROM detallecompra d INNER JOIN facturacompra f ON d.factura=f.factura GROUP BY factura";
    ...
    case 'allClientsCredit':
        $sql = "SELECT * FROM (
            SELECT cliente FROM credito GROUP BY cliente
        ) as c INNER JOIN clientes AS cli ON c.cliente= cli.nombrecli";
        $query = querySimple($sql);
        $outpout = [];
        while ($client = mysqli_fetch_object($query)) {
            $estado = '';
            ...
            $btnViewCredits = '<a href="viewCredits.php?nombrecli=' . $client->nombrecli . '" class="btn btn-dark px-2 py-2"><i class="fa fa-eye"></i> Ver Creditos</a>';

            $outpout[] = [
                '0' => $client->nombrecli,
                '1' => $client->cedula,
                '2' => empty($client->celular) ? '<span class="text-muted">No definido</span>' : $client->celular,
                '3' => empty($client->direc) ? '<span class="text-muted">No definido</span>' : $client->direc,
                '4' => $btnViewCredits
            ];
        }
        include_once '../helper/processAjax.php';
    ...
    case 'allCreditsOwnerClient':
        ...
        $sql = "SELECT
                    ...
                    cred.interes,
                    cred.cliente,
                    cred.total_con_interes as totalInteres,
                    cred.total_cuotas as cuotas,
                    cred.saldo,
                    cred.estado,
                    cred.fecha_fin,
                    det.valor,
                ...
                INNER JOIN detalle as det
                ON cred.detalle_id=det.id
                WHERE cliente='$nombrecli'";
        $query = querySimple($sql);
        $outpout = [];
        while ($credit = mysqli_fetch_object($query)) {
            // $btnViewCredits = '<a href="viewCredits.php?nombrecli=' . $credit->cliente . '&credito=' . $credit->credito_id . '" target="admin" id="modal-cuotas" data-toggle="modal" data-target="#cuotasModal" class="btn btn-dark px-2 py-2"><i class="fa fa-eye"></i> Detalles y Cuotas</a>';
            $btnViewCredits = "<a
                href='viewCuotas.php?credito_id={$credit->credito_id}&prod={$credit->nombre}&saldo={$credit->saldo}'
                target='admin' class='btn btn-dark px-2 py-2'>
                    <i class='fa fa-eye'></i>
                    Detalles y Cuotas
                </a>";
            ...
            $outpout[] = [
                '0' => $credit->nombre,
                '1' => number_format($credit->totalInteres, 0, ',', '.') .
                    ' Gs' . ' - <span class="text-danger font-weight-bold">(' . $credit->interes . '%)</span>',
                '2' => $credit->cuotas,
                '3' => $credit->saldo,
                '4' => $credit->estado == 'PENDIENTE'
                    ? '<span class="badge badge-info">Pendiente</span>'
                    : '<span class="badge badge-success">Pagado</span>',
                '5' => Carbon::parse($credit->fecha_fin)->toFormattedDateString(),
                '6' => $btnViewCredits
            ];
        }
        include_once '../helper/processAjax.php';
